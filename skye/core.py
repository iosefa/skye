...
class SkyView:
    def __init__(
        ...
        self.img = create_hemispherical(img)
        if enhance_image:
            self.img = enhance(self.img)
        if adjust_gamma:
            self.img = gamma_correction(self.img, gamma=gamma)
        logging.info('Segmenting image...')
        self.segments = segment_image(self.img, ratio=segmentation_ratio, sigma=sigma)
        self.segmented_img = self.create_segmented_img()
        self.classified_img = None
        self.sky_view_factor = None
        self.objects = create_objects(self.img, self.segments)
    ...
    def create_training_data(self, n_samples=500, notebook=False, save_segment_path=None):
        ...
        for j, i in enumerate(sample):
            ...
            while True:
                klass = input("Enter class ('0' for non-sky, '1' for sky) or type 'skip' to skip. To quit, enter 'I give up'.")
            ...
            if save_segment_path:
                # todo: get name from metadata instead
                name = self.image_path.split('/')[-1].split('.')[0]
                segment_fig.savefig(f"{save_segment_path}/{name}_{i}_{j}_r{len(self.training_classes)}_cl{klass}.jpg", dpi=72)
            if notebook:
                clear_output(wait=True)
        print('Congratulations! You did it!')
